...
export default {
    ...
    methods: {
        ...
        changeDispOrd(item){
          ...
          let index = this.selectedTdmotion.indexOf(this.selectedTdmotion.find(f => f.motion_id === item.motion_id))
          Object.assign(this.selectedTdmotion[index], item)
          
          //console.log('this.selectedTdmotion : ' + JSON.stringify(this.selectedTdmotion, null, 2))          
        },

        checkDispOrd(){     
          for(const item of this.selectedTdmotion)
          {
            if(!item.disp_ord){
                alert(item.name + ' : 표시순서를 입력하세요')
              return false
            }
          }
          return true
