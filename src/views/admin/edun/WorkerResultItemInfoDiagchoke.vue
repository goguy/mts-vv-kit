                ...
                >
                    ...
                    <v-sheet class="mt-5">
                        ...
                        <tbody>
                            ...
                            <td width="40%" class="text-left text-caption"> 작업 중 산소농도 측정여부</td>                
                            <td width="20%" class="text-center"> 15점</td>                
                            <td width="20%" class="text-center"> {{oxygenWorkScore}}</td>                
                            </tr>
                            <tr>
                            ...
                            <td width="40%" class="text-left text-caption"> 돌발상황 발생 인지여부</td>                
                            <td width="20%" class="text-center"> 20점</td>                
                            <td width="20%" class="text-center"> {{outbreakRecognizeScore}}</td>                
                            </tr>
                            <tr>
...
export default {
    ...
    name: 'WorkerResultInfoDiagchoke',
    props: {
        dialogParam: Object,
    },
    data: () => ({
    ...
    computed: {
        ...
        oxygenInitScore() {
            return this.getResultItemScore('equip_check')
        },
        oxygenWorkScore() {
            return this.getResultItemScore('work_check')
        },
        workTimeScore() {
            return this.getResultItemScore('work_time')
        },
        outbreakRecognizeScore() {
            return this.getResultItemScore('outbreak_recognize')
        },
        outbreakCopeScore() {
            return this.getResultItemScore('outbreak_cope')
        },
        totalScore(){
        //총점 100점
    ...
    methods: {
        ...mapActions(workerResultEvalStore, [
            'AC_WORKER_RESULT_EVAL', 'AC_WORKER_RESULT_EVAL_PUSH', 
            'AC_WORKER_RESULT_EVAL_EDIT', 'AC_WORKER_RESULT_EVAL_DEL', 'AC_WORKER_RESULT_EVAL_LIST' 
        ]),
        ...
        setWorkerResultEvalList() {
            ...
            if(query.length >0){
                this.editedIndex = query[0].result_id
                this.editedItem.rmk=query[0].rmk
                this.editedItem.is_pass=query[0].is_pass
            }
        },
        ...
        save () {
            ...
            this.editedItem.worker_id = this.dialogParam.worker_id
            this.editedItem.work_id = this.dialogParam.work_id
            this.editedItem.result_id = this.dialogParam.result_id
            this.editedItem.user_email = this.GE_USER.email

            if (this.editedIndex > -1) {
                this.payload.index=this.editedIndex
                this.payload.workerResultEval=this.editedItem
                this.AC_WORKER_RESULT_EVAL_EDIT(this.payload)
                console.log('this.editedIndex : ' + this.editedIndex)
                this.setSnackbar('업데이트')
            }else{                                
                this.AC_WORKER_RESULT_EVAL_PUSH(this.editedItem)
                //this.setWorkerResultEvalList()
                this.editedIndex=0
                this.setSnackbar('등록')
            }    
