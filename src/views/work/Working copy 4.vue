...
export default {
  ...
  watch: { 
     playerListCurrentIndex(val){
      ...
      if( (val+1) === this.videoListQuery('normal') ){
        let outbreakTime = this.timeWork + 174
        this.setOutbreak(outbreakTime)
        //console.log('aaaaaa',this.videoListQuery('normal'))
      }
    },
  ...
  mounted() {
    ...
    if(this.response){ 
      ...
      //motion-list
      this.playerSelectedItem = this.player.playlist.currentItem()      
    }
  ...
  methods: {
    ...
    async playVideoPlayer(type){
      ...
      if(this.videoList.length > 0){
        ...
        if(type === 'normal'){
          this.player.playlist.currentItem(this.playerSelectedItem)
          
          if(this.checkOutbreakBeforePlay){
            console.log('playVideoPlayer - checkOutbreakBeforePlay', this.checkOutbreakBeforePlay)
            setTimeout(() => (
              this.player.play()
            ), 1)    
          }else{
          ...
          this.checkOutbreakBeforePlay=false
    ...
    btnNomalVideoPlayer() {
      ...
      console.log('this.playerSelectedItem', this.playerSelectedItem)
    },
    //산소농도건강장애
